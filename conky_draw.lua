require 'cairo'
dofile(os.getenv("HOME") .. '/.conky/conky_draw_config.lua')
...
-- Added by me (selph)
-- Literally only used to draw the orbs
function draw_orb(display, element)
    -- the user types degrees, but we need radians
    local start_angle, end_angle = math.rad(element.start_angle), math.rad(element.end_angle)
    ...
    -- set gradient color
    pattern = cairo_pattern_create_radial(element.center.x, element.center.y, 50, element.center.x, element.center.y, element.radius)
    cairo_pattern_add_color_stop_rgba(pattern, 0, hexa_to_rgb(element.color, 0))
    cairo_pattern_add_color_stop_rgba(pattern, 1, hexa_to_rgb(element.color, 0.35))
    cairo_set_source(display, pattern)
    -- draw the ring
    arc_drawer(display, element.center.x, element.center.y, element.radius, start_angle, end_angle)
    cairo_fill(display);
end
